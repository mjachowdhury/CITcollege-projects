 ...
 if(check(user) && check(pwd) && check(cpwd) && isPasswordMatch(frm1) && check(email) && isValidSymbol(frm1) && isValidDomain(frm1) && check(dob) && MenuSelected(hqo) && check(hqa) &&  check(addr) && MenuSelected(occp) && check(zodiac)) return true;
 return false;
 }
