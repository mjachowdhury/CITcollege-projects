...
public class OnsiteDeparture extends HttpServlet
{
        ...
        throws ServletException, IOException
    {
        ...
        try
        {
            ...
            String s15 = "select max(OnSiteID) from OnsiteDeparture";
            ...
            } else
            {
                ...
                {
                    s17 = (new StringBuilder()).append("Dep").append(Integer.parseInt(s17.substring(3, s17.length())) + 1).toString();
                }
            }
        ...
        {
            System.out.println((new StringBuilder()).append("Error While Generating Onsite Departure ID : ").append(exception).toString());
        }
        try
        {
            String s16 = (new StringBuilder()).append("insert into OnsiteDeparture values('").append(s17).append("','").append(s).append("','").append(s14).append("','").append(s1).append("','").append(s2).append("','").append(s3).append("','").append(s4).append("','").append(s5).append("','").append(s9).append("','").append(s13).append("','").append(date).append("')").toString();
            Database database1 = new Database();
            int i = database1.setResultSet(s16);
            s16 = (new StringBuilder()).append("update ApplicationEntry set OnsiteDeparture='yes' where ApplnID='").append(s).append("'").toString();
            i = database1.setResultSet(s16);
            database1.closeSetResultSet();
        }
        catch(Exception exception1)
        {
            System.out.println((new StringBuilder()).append("Error While Inserting OnsiteDeparture : ").append(exception1).toString());
        }
        httpservletresponse.sendRedirect("HrManager/OnsiteDeparture.jsp");
    }
